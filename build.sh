...
# 编译 bash
curl -fLO https://ftp.gnu.org/gnu/bash/bash-5.3.tar.gz
tar -zxf bash-5.3.tar.gz
cd bash-5.3
./configure --prefix=/opt/bash-5.3-ohos-arm64 --with-bash-malloc=no
make -j$(nproc)
make install
cd ..

# 进行代码签名
cd /opt/bash-5.3-ohos-arm64
...
cp bash-5.3/COPYING /opt/bash-5.3-ohos-arm64/
cp bash-5.3/AUTHORS /opt/bash-5.3-ohos-arm64/

# 打包最终产物
cp -r /opt/bash-5.3-ohos-arm64 ./
tar -zcf bash-5.3-ohos-arm64.tar.gz bash-5.3-ohos-arm64
